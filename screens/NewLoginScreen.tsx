    ...
    const handleEmailAction = async () => {
        ...
        if (forgotPasswordMode) {
            try {
                ...
                Alert.alert(t('common:success'), t('auth:email.resetSent'));
                setForgotPasswordMode(false);
            } catch (error) {
                Alert.alert(t('common:error'), t('auth:email.resetError'));
            } finally {
                setIsLoading(false);
            }
            return;
        ...
        try {
            ...
            if (isLoginMode) {
                ...
                // REGISTER
                // Check if user exists logic could be here, but firebase handles it
                const fbUser = await signUpWithEmail(email, password);
                await sendVerification(fbUser);

                Alert.alert(
                    t('auth:email.verifyTitle'),
                    t('auth:email.verifySent'),
                    [{ text: t('common:confirm'), onPress: () => setIsLoginMode(true) }]
                );
            }
        } catch (error: any) {
            console.error('Auth action failed', error);
            let msg = t('common:genericTryAgain');
            if (error.code === 'auth/email-already-in-use') msg = t('auth:email.knownUser');
            if (error.code === 'auth/wrong-password') msg = t('auth:email.invalidPassword');
            if (error.code === 'auth/user-not-found') msg = t('auth:email.unknownEmail');
                ...
                >
                    ...
                    <View style={styles.header}>
                        ...
                        <Text style={styles.subtitle}>
                            {isLoginMode ? t('common:welcomeShort') : t('auth:subtitle')}
                        </Text>
                    </View>
                        ...
                        <View style={styles.footerActions}>
                            {!forgotPasswordMode && (
                                <TouchableOpacity onPress={() => setForgotPasswordMode(true)}>
                                    <Text style={styles.linkText}>{t('auth:email.resetPassword')}</Text>
                                </TouchableOpacity>
                            )}
