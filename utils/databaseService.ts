...
export const db = {
      ...
      if (USE_BACKEND) {
        ...
        const payload = {
          ...
          metadata: { 
            ...
            recurrenceFrequency: rideData.recurrenceFrequency || null,
            recurrenceUnit: rideData.recurrenceUnit || null
          },
        };
  ...
  listRides: async (_userId: string, options: { includePast?: boolean } = {}) => {
    try {
      if (USE_BACKEND) {
        const enhancedDB = await loadEnhancedDB();
        const apiRides = await enhancedDB.getRides({ include_past: options.includePast });
  ...
  getUserRides: async (userId: string, type: 'driver' | 'passenger' = 'driver') => {
    try {
      if (USE_BACKEND) {
        let enhancedDB = await loadEnhancedDB();
        let apiRides = [];

        // Check if method exists. If not, fallback to direct API call.
        if (typeof enhancedDB.getUserRides === 'function') {
          apiRides = await enhancedDB.getUserRides(userId, type);
        } else {
          console.warn('⚠️ getUserRides missing on enhancedDB (stale instance), bypassing cache and using apiService directly');
          const response = await apiService.getUserRides(userId, type);
          apiRides = response.success ? response.data : [];
        }
      ...
      // Fallback for local/firebase mock if needed, or just return empty
      return DatabaseService.list(DB_COLLECTIONS.RIDES, userId);
    } catch (error) {
  ...
  updateRide: async (_userId: string, rideId: string, rideData: Partial<any>) => {
    try {
      if (USE_BACKEND) {
        ...
        const res = await apiService.updateRide(rideId, payload);
  ...
  listAllLinks: async (): Promise<any[]> => {
    try {
      if (USE_BACKEND) {
        const links = await restAdapter.listAll(DB_COLLECTIONS.LINKS);
        console.log('📥 DatabaseService - ListAllLinks received:', links);
        console.log('📥 DatabaseService - ListAllLinks count:', links?.length || 0);
        return links || [];
      } else if (USE_FIRESTORE) {
        // For Firestore, we'd need to implement a query that gets all links
        // For now, return empty array
        return [];
      } else {
        // For AsyncStorage, get all keys that start with 'links_'
        const keys = await AsyncStorage.getAllKeys();
      ...
      console.error('❌ DatabaseService - ListAllLinks error:', error);
      return [];
    }
  },
};
